...
void GLSLProgram::UniformMat4( unsigned int uniform_id,const float* mat )
{
    glUniformMatrix4fv( uniforms[ uniform_id ], 1, GL_FALSE, mat );
}

void GLSLProgram::UniformVec3( unsigned int uniform_id, const float* v )
{
    glUniform3f( uniforms[ uniform_id ], v[0], v[1], v[2] );
}

void GLSLProgram::UniformMat4Array( unsigned int uniform_id, unsigned int count, const float* mat )
{
    glUniformMatrix4fv( uniforms[ uniform_id ], count, GL_FALSE, mat );
}
void GLSLProgram::UniformMat3Array( unsigned int uniform_id, unsigned int count, const float* mat )
{
    glUniformMatrix3fv( uniforms[ uniform_id ], count, GL_FALSE, mat );
}
